...
import com.example.credrdummy.model.*
import com.example.credrdummy.service.FranchiseWebServiceProvider
import com.example.credrdummy.service.WebServiceProvider
...
class LogisticVehicleStatusActivity : AppCompatActivity() {


    private lateinit var searchView: SearchView
    private var index:Int=1
    private var loading:Boolean=false
    private lateinit var apiProvider: WebServiceProvider
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        apiProvider = WebServiceProvider.retrofit.create(WebServiceProvider::class.java)
    ...
    private fun filerList(list:ArrayList<VehicleStatu>, status:String):ArrayList<VehicleStatu>{
        val listFilter = ArrayList<VehicleStatu>()
        for(l in list){
    ...
    private fun fetchBikeList() {
        ...
        apiProvider.getVehicleStatus(user).observeOn(AndroidSchedulers.mainThread())
                ?.subscribe(object : SingleObserver<LogistisVehicleResponse> {
                    override fun onSuccess(r: LogistisVehicleResponse) {

                        Log.d("Ok>>>","onSuccess")
                        progressBar.visibility = View.GONE
                        mDataList.clear()
                        mDataList.addAll(r.vehicleStatus)
   ...
   /* private fun setRecyclerViewScrollListener() {
    ...
    inner class PagerAdapter(fm: FragmentManager,var list:ArrayList<VehicleStatu>) : FragmentPagerAdapter(fm) {

        private val mTabArray = arrayOf("All", "Assigned", "Pending", "In Transit", "Delivered","Dispute")

        private  val franArray = arrayOf(
            LogistictsFragmentVehicleListStatus.newInstance(
                list
            ),
            LogistictsFragmentVehicleListStatus.newInstance(
                filerList(
